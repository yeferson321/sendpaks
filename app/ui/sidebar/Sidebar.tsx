...
import { changeTheme } from '../../redux/features/theme/theme';

import { defineLanguage } from '@/app/lib/ola';

type ProviderProps = {
    children: React.ReactNode;
};

const text = defineLanguage()
            ...
            <ul className="space-y-2 font-medium">
                ...
                <li>
                    <Link href="#" className="flex items-center w-full p-1.5 text-gray-600 dark:text-slate-300 rounded-lg hover:bg-black/10 dark:hover:bg-slate-100/10 group">
                        ...
                        <span className="flex-1 ms-3 whitespace-nowrap">{text.sidebar.about}</span>
                    </Link>
                </li>
                <li>
                    <button type="button" onClick={() => dispatch(changeTheme())} className="flex items-center w-full p-1.5 text-gray-600 dark:text-slate-300 rounded-lg hover:bg-black/10 dark:hover:bg-slate-100/10 group">
                        {theme ? (
                            <>
                                ...
                                <span className="flex-1 ms-3 whitespace-nowrap">{text.sidebar.dark_mode}</span>
                            </>
                        ) : (
                            <>
                                ...
                                <span className="flex-1 ms-3 whitespace-nowrap">{text.sidebar.light_mode}</span>
                ...
                <li>
                    <Link href="#" className="flex items-center w-full p-1.5 text-gray-600 dark:text-slate-300 rounded-lg hover:bg-black/10 dark:hover:bg-slate-100/10 group">
                        ...
                        <span className="flex-1 ms-3 whitespace-nowrap">{text.sidebar.login}</span>
                    </Link>
                </li>
            </ul>
        </aside>
